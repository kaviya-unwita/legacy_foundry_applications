...
function FieldMappingDisclosure({ module, master }) {
  ...
  return <div className={`field-mapping-disclosure ${open ? 'open' : ''}`}>
    <button className="field-mapping-toggle" type="button" aria-expanded={open} onClick={() => setOpen(!open)}>
      <span><Database/><span><strong>Field mapping & future use</strong><small>{mappings.length} fields · {counts.confirmed ?? 0} confirmed in both · {counts.partial ?? 0} partial · {counts.proposed ?? 0} proposed</small>{!counts.confirmed && !counts.partial && <em className="brand-new-master">Brand-new Phoenix master</em>}</span></span><ChevronDown/>
    </button>
