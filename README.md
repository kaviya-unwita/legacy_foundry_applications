...
- **Layout like the legacy screen:** each page is arranged as the legacy form draws it. Sections are the form's own boxes (frames and rectangles, split by divider lines), titled with the text on their top edge. Boxes that sit side by side on the legacy screen sit side by side here. Inside a section, every field keeps its legacy row and column, so a two-column section stays two columns and fields that share a legacy row (e.g. City and Pin Code) share a row. Labels sit where the legacy draws them: left of the input, above it (grid columns and matrices such as YS MIN / YS MAX), or right of a check box. The layout is decoded from the `.fmx` boilerplate graphics and item coordinates. Each screenshot is aligned to the form by the texts they share, so screenshot-only fields are placed where they appear in the screenshot.
