...
- **Sections and field order:** each page is divided as the legacy form divides it, into one card per legacy section (the form's own frames and rectangles, split by divider lines, titled with the text on their top edge). Cards follow the legacy reading order: top to bottom, left to right. Fields inside a card follow the legacy order. A section with two or more tall legacy columns (Customer, Company) is read down each column. Anything else (a row of check boxes, a MIN/MAX matrix) is read across each row. The look is a plain modern form, not a copy of the old screen: labels sit above the inputs, long legacy fields (addresses, remarks) take two grid cells, and check boxes keep their label beside the box. The sections and order are decoded from the `.fmx` boilerplate graphics and item coordinates. Each screenshot is aligned to the form by the texts they share, so screenshot-only fields fall in the right place in the order.
- **Readability:** each field shows a small coloured dot for its source, with the full text on hover. The **Field sources** button shows the full tags. The F9 list button appears when you point at or type in a field.
- **Field names:** the label is what the legacy screen shows: the item's prompt, or the text drawn beside it. For an item the form leaves unlabelled, it is the screenshot text beside the field. A different screenshot text beside an already labelled item comes from a newer version of the screen that moved things, so it never renames that item. Screenshot labels that are really section headings, or duplicates of a field beside them, are not shown as fields.
...
- **Checked by eye:** all 186 SUN's pages that have a screenshot were compared with their screenshot by eye. On each one, every field the screenshot shows is on the page, under the screenshot's label, in the screenshot's order and sections. A field the screenshot shows is tagged form + screenshot (or screenshot only when the form file has no item for it). A section title is used only where the screenshot has a real heading, never a field label. Legacy items the screenshot does not show (pop-ups, columns scrolled out of view, older fields) go in a last card, **In the legacy form, not in the screenshot**. Buttons are not fields. When two screenshots show the same form (Group Master and its Delivery Delay pop-up, P.O. Edit scrolled left and right), each page holds the fields its own screenshot shows. Each field keeps its own value, even when two fields on a page have the same label.
